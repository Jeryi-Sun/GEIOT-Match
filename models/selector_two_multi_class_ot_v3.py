...
parser.add_argument('--threshold_ot', type=float, default=1.47, help='threshold ot')
parser.add_argument('--f_mass', type=float, default=3, help='f mass')
parser.add_argument('--weight', type=float, default=0, help='weight')
...
parser.add_argument('--OPW_weight', type=float, default=0.01,  help='OPW_weight')
...
data_extract_json = '/home/zhongxiang_sun/code/explanation_project/explanation_model/dataset/our_data/data_extract.json'
data_extract_npy = '/home/zhongxiang_sun/code/explanation_project/explanation_model/dataset/our_data/data_extract.npy'
...
def save_checkpoint(model,optimizer, trained_epoch, ot_model=None):
    ...
    filename = args.checkpoint + '/' + "extract-criterion-{}-onehot-{}-ot_mode-{}-convert_to_onehot-{}-weight-{}-simot-{}-simpercent-{}.pkl".format(args.criterion, args.convert_to_onehot, args.ot_mode, args.convert_to_onehot,
    ...
    if ot_model!=None:
        ...
        filename = args.checkpoint + '/' + "extract_ot-criterion-{}-onehot-{}-ot_mode-{}-convert_to_onehot-{}-weight-{}-simot-{}-simpercent-{}.pkl".format(args.criterion, args.convert_to_onehot, args.ot_mode, args.convert_to_onehot,
                                                                                                 args.weight, args.simot, args.simpercent)
...
class Selector2_mul_class(nn.Module):
    def __init__(self, input_size, filters, kernel_size, dilation_rate):
        ...
        self.dense2 = nn.Linear(filters, 4)

    def forward(self, inputs):
        mask = torch.abs(inputs).ge(0.00001)
...
class OT(nn.Module):
    ...
    def forward(self, case_A_logits, case_B_logits, case_A, case_B, relation_matrix, padding_mask_a, padding_mask_b, convert_to_onehot=True, convert_mode='max', model_type="train"):
        ...
        padding_mask_value = -10*padding_mask + 10    # 1e9 跟bert一样 算loss的时候将这部分梯度截断
        case_A = self.embedding_transformer(case_A)
        case_B = self.embedding_transformer(case_B)
        """
        加一个norm_2 之后跑一下
        # cost metric 操作一手，放到OPW中计算距离，同时计算一下GW OT的距离
        """
        cost_orig = OT_utils.cost_matrix(case_A, case_B, cost_type="cosine")
        Order_preserve_cost = OT_utils.cost_matrix(case_A, case_B, cost_type="OPW_distance", lambda_1=1, lambda_2=0.1, sigma=1)
        Cs = OT_utils.cost_matrix(case_A, case_A, cost_type="cosine")
        Ct = OT_utils.cost_matrix(case_B, case_B, cost_type="cosine")
        if not convert_to_onehot:
            # 想要让对应位置plan变大，就要让W矩阵对应位置减小，那么就要让DGCNN的输出概率增大， 最大可以大到使得W（cost）变为 0
            class_mask_value = -(args.weight + cost_orig) * class_mask + args.weight  # cdist2 最大根号下786（bert输出维度） 算loss的时候将这部分梯度留下
        else:
            class_mask_value = -args.weight * class_mask + args.weight  # cdist2 最大根号下786（bert输出维度） 算loss的时候将这部分梯度留下

        if model_type == 'train':
            cost_matrix_class = cost_orig + self.weight*class_mask_value + padding_mask_value  # 即使两个padding都有的位置也是存在的，合理的
        else:
        ...
        return_plan_class, _ = OT_utils.GW_alg(Cs, Ct, Other_C=args.OPW_weight*Order_preserve_cost+cost_matrix_class) # 增加图距离
...
def train_pair(args, model, OT_model, train_dataloader, valid_dataloader, test_dataloader):
    ...
    for epoch in range(args.epoch_num):
        ...
        for batch_data in pbar:
            ...
            current_step += 1
            pbar.set_description("dgcnn loss {:.8}, ot loss {:.8}".format(epoch_loss_dgcnn / current_step,
...
def evaluate(model, OT_model, valid_dataloader, epoch, type='valid'):
    ...
    for batch_data in pbar:
        ...
        取出非mask的部分
        """
        batch_len_A = torch.sum(batch_mask_A, dim=-1)
        batch_len_B = torch.sum(batch_mask_B, dim=-1)
        ...
        label_batch_A_list = label_batch_A.cpu().tolist()
        label_batch_B_list = label_batch_B.cpu().tolist()

        for i in range(label_batch_A.shape[0]):
            four_class_labels += label_batch_A_list[i][:batch_len_A[i]]
            four_class_labels += label_batch_B_list[i][:batch_len_B[i]]
        """
        七类别标签
        """
        ...
        # if type=='test':
        #     for i in range(len(x_batch_A)):
        #         padding_mask_matrix = padding_mask[i]
        #         len_y = torch.sum(padding_mask_matrix[0, :]).detach().cpu().long()
        #         len_x = torch.sum(padding_mask_matrix[:, 0]).detach().cpu().long()
        #         conf_matrix = plan_list[i][:len_x, :len_y].detach().cpu().numpy()
        #         x = plan_list[i][:len_x, :len_y]
        #         torch.save(x, 'IOT_match.pt')
        ...
        #
        #         plt.xlabel('Case A', fontsize=13)
        #         plt.ylabel('Case B', fontsize=13)
        #         plt.title('Heat Map', fontsize=18)
        #         plt.show()
        #
        #         conf_matrix = relation_matrix_batch[i][:len_x, :len_y].detach().cpu().numpy()
        #         x = relation_matrix_batch[i][:len_x, :len_y]
        #         torch.save(x, 'relation_label.pt')
        #         fig, ax = plt.subplots(figsize=(6, 6))
        #         ax.matshow(conf_matrix, cmap=plt.cm.Oranges, alpha=0.3)
        #         # for i in range(conf_matrix.shape[0]):
        #         #     for j in range(conf_matrix.shape[1]):
        ...
        #         plt.xlabel('Case A', fontsize=13)
        #         plt.ylabel('Case B', fontsize=13)
        #         plt.title('Label_map', fontsize=18)
        #         plt.show()
        #         exit()
        ...
        four_prediction_A = torch.argmax(output_batch_A, dim=-1).long()
        four_prediction_B = torch.argmax(output_batch_B, dim=-1).long()
        four_prediction_A_list = four_prediction_A.cpu().tolist()
        four_prediction_B_list = four_prediction_B.cpu().tolist()
        for i in range(label_batch_A.shape[0]):
            four_class_predictions += four_prediction_A_list[i][:batch_len_A[i]]
            four_class_predictions += four_prediction_B_list[i][:batch_len_B[i]]
    ...
    accuracy = accuracy_score(four_class_labels, four_class_predictions)
    precision_macro = precision_score(four_class_labels, four_class_predictions, average='macro')
    recall_macro = recall_score(four_class_labels, four_class_predictions, average='macro')
    f1_macro = f1_score(four_class_labels, four_class_predictions, average='macro')
    time_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(colored('{} {}  acc {} precision ma {} recall  ma {} f1  ma {} '.format(time_str, type, accuracy,
                                                                                  precision_macro,
                                                                                  recall_macro,
                                                                                  f1_macro), 'red'))
...
def model_class(batch_A, batch_B, plan_matrix):
    ...
    vec_correct_A = torch.argmax(batch_A, dim=-1) + (relation_A*3)*torch.ge(torch.argmax(batch_A, dim=-1), 1)
    vec_correct_B = torch.argmax(batch_B, dim=-1) + (relation_B*3)*torch.ge(torch.argmax(batch_B, dim=-1), 1)
...
def OT_param_search(model, valid_dataloader, test_dataloader):
    ...
    reg:    [0.3 - 1.] 7
    """
    threshold_ot_list = list(np.arange(1, 1.5, 0.01))
    weight_list = [0, 5, 10]
    # f_mass_list = [2]
    # reg_list = list(np.arange(0.1, 1, 0.1))
    pd_dic_valid = {"threshold_ot": [], "weight": [], "DGCNN_Acc": [], "OT_Acc": [],
                    "OT_Pre": [], "OT_Recall": [], "OT_F1": [],'seven_class_DGCNN':[]}
    pd_dic_test = {"threshold_ot": [], "weight": [], "DGCNN_Acc": [], "OT_Acc": [],
                   "OT_Pre": [], "OT_Recall": [], "OT_F1": [], 'seven_class_DGCNN':[]}
    filename_OT = args.checkpoint + '/' + "extract_ot-criterion-{}-onehot-{}-ot_mode-{}-convert_to_onehot-{}-weight-{}-simot-{}-simpercent-{}.pkl".format(
    ...
    pd_valid.to_csv("../dataset/our_data/valid_unsup_{}_semi_percent_{}.csv".format(args.simot, args.simpercent), index=False)
    pd_test.to_csv("../dataset/our_data/test_unsup_{}_semi_percent_{}.csv".format(args.simot, args.simpercent), index=False)
...
if __name__ == '__main__':
    ...
    for i, d in enumerate(data):
        ...
        for j in case_a[1]:
            if j[0] in important_A:
                data_y_seven_class[2*i, j[0]] = j[1]+3
            else:
                data_y_seven_class[2*i, j[0]] = j[1]

        for j in case_b[1]:
            if j[0] in important_B:
                data_y_seven_class[2*i+1, j[0]] = j[1]+3
            else:
    ...
    temp use
    """
    # train_data_x = data_split(data_x, 'train')
    # valid_data_x = data_split(data_x, 'valid')
    # test_data_x = data_split(data_x, 'test')
    # data_11 = test_data[6]
    # data_x_11 = test_data_x[11:13]
    # with open("/new_disk2/zhongxiang_sun/code/explanation_project/explanation_model/dataset/our_data/heat_map/temp.json", 'w') as f:
    #     for i in range(10):
    #         f.writelines(json.dumps(data_11, ensure_ascii=False))
    #         f.write('\n')
    # np.save("/new_disk2/zhongxiang_sun/code/explanation_project/explanation_model/dataset/our_data/heat_map/temp.npy", np.tile(data_x_11, (10, 1, 1)))

    #
    # torch.save(torch.tensor(data_matrix[0][:9, :13]), 'relation_label.pt')
    # exit()
    ...
    train_x_A = data_split(data_x_A, 'train')
    train_x_B = data_split(data_x_B, 'train')
    train_y_A = data_split(data_y_A, 'train')
    train_y_B = data_split(data_y_B, 'train')
    train_y_seven_class_A = data_split(data_y_seven_class_A, 'train')
    train_y_seven_class_B = data_split(data_y_seven_class_B, 'train')

    train_relation_matrix = data_split(data_matrix, 'train')
    valid_x_A = data_split(data_x_A, 'valid')
    valid_x_B = data_split(data_x_B, 'valid')
    valid_y_A = data_split(data_y_A, 'valid')
    valid_y_B = data_split(data_y_B, 'valid')
    valid_y_seven_class_A = data_split(data_y_seven_class_A, 'valid')
    valid_y_seven_class_B = data_split(data_y_seven_class_B, 'valid')
    valid_relation_matrix = data_split(data_matrix, 'valid')
    test_x_A = data_split(data_x_A, 'test')
    test_x_B = data_split(data_x_B, 'test')
    test_y_A = data_split(data_y_A, 'test')
    test_y_B = data_split(data_y_B, 'test')
    test_y_seven_class_A = data_split(data_y_seven_class_A, 'test')
    test_y_seven_class_B = data_split(data_y_seven_class_B, 'test')
    test_relation_matrix = data_split(data_matrix, 'test')
    ...
    if args.model_usage == 'train':
        ...
        filename_DGCNN = args.checkpoint + '/' + "extract-criterion-{}-onehot-{}-ot_mode-{}-convert_to_onehot-{}-weight-{}-simot-{}-simpercent-{}.pkl".format(args.criterion, args.convert_to_onehot, args.ot_mode, args.convert_to_onehot,
                                                                                                 args.weight, args.simot, args.simpercent)
        ...
        filename_DGCNN = args.checkpoint + '/' + "extract-criterion-{}-onehot-{}-ot_mode-{}-convert_to_onehot-{}-weight-{}-simot-{}-simpercent-{}.pkl".format(
            args.criterion, args.convert_to_onehot, args.ot_mode, args.convert_to_onehot,
            args.weight, args.simot, args.simpercent)
        load_checkpoint(model, None, 22, filename_DGCNN)
        OT_model = OT(reg=args.reg).to(device)

...
parser.add_argument('--coefficient', type=float, default=10., help='balance the dgcnn loss and the ot loss')
...
parser.add_argument('--threshold_ot', type=float, default=1.1, help='threshold ot')
...
parser.add_argument('--reg', type=float, default=0.5, help='regular')
...
def save_checkpoint(model,optimizer, trained_epoch, ot_model=None):
    ...
    filename = args.checkpoint + '/' + "cail_extract-criterion-{}-onehot-{}-ot_mode-{}-four_class-{}-coefficient_lrable-{}-convert_to_onehot-{}-loss_only_label1-{}-weight-{}-cost_cal_way-{}.pkl".format(args.criterion, args.convert_to_onehot, args.ot_mode, args.four_class, args.coefficient_lrable, args.convert_to_onehot,
                                                                                                args.loss_only_label1, args.weight, args.cost_cal_way)
    ...
    if ot_model!=None:
        ...
        filename = args.checkpoint + '/' + "cail_extract_ot-criterion-{}-onehot-{}-ot_mode-{}-four_class-{}-coefficient_lrable-{}-convert_to_onehot-{}-loss_only_label1-{}-weight-{}-cost_cal_way-{}.pkl".format(args.criterion, args.convert_to_onehot, args.ot_mode, args.four_class, args.coefficient_lrable, args.convert_to_onehot,
                                                                                                args.loss_only_label1, args.weight, args.cost_cal_way)
...
class OT(nn.Module):
    def __init__(self, reg=0.5, mass=0.5, numItermax=10):
        ...
        if args.cost_cal_way == 'attention':
            self.cost_matrix = nn.init.xavier_normal_(nn.Parameter(torch.empty(args.input_size, args.input_size)))
        elif args.cost_cal_way == 'class_embedding':
            self.cost_matrix = nn.init.xavier_normal_(nn.Parameter(torch.empty(args.input_size, args.input_size)))
            self.class_embedding = nn.Embedding(2, args.input_size)
            self.class_embedding.weight.data = nn.init.xavier_normal_(self.class_embedding.weight.data)
    ...
    def convert_one_hot(self, logits, temperature=1., hard=False, mode='max'):
        """
        :param logits:
        :param temperature:
    ...
    def forward(self, case_A_logits, case_B_logits, case_A, case_B, relation_matrix, padding_mask_a, padding_mask_b, convert_to_onehot=True, convert_mode='max'):
        ...
        if convert_to_onehot:
            case_A_onehot = self.convert_one_hot(case_A_logits, hard=True, mode=convert_mode)
            case_B_onehot = self.convert_one_hot(case_B_logits, hard=True, mode=convert_mode)
        else:
        ...
        if args.cost_cal_way == 'l2':
            cost_orig = torch.cdist(case_A, case_B)
        elif args.cost_cal_way == 'attention':
            cost_orig = torch.bmm(torch.matmul(case_A, self.cost_matrix), case_B.permute([0, 2, 1]))  # todo 加入优化器
        elif args.cost_cal_way == 'class_embedding':
            DGCNN_label_A = torch.argmax(case_A_logits, dim=-1)
            DGCNN_label_B = torch.argmax(case_B_logits, dim=-1)
            case_A = torch.matmul(case_A, self.cost_matrix)
            case_B = torch.matmul(case_B, self.cost_matrix)
            cla_emb_A = self.class_embedding(DGCNN_label_A)
            cla_emb_B = self.class_embedding(DGCNN_label_B)
            case_A = torch.cat([case_A, cla_emb_A], dim=-1)
            case_B = torch.cat([case_B, cla_emb_B], dim=-1)
            cost_orig = torch.cdist(case_A, case_B)
        else:
            print("cost cal way is in [l2, attention, class_embedding]")
            exit()
        ...
        cost_matrix = cost_orig + class_mask_value + padding_mask_value  # 即使两个padding都有的位置也是存在的，合理的
        ...
        for i in range(case_A.shape[0]):
            ...
            plan = sinkhorn_knopp(margin_A, margin_B, cost_matrix[i], reg=self.reg, maxIter=self.numItermax) # , m=self.mass
            plan_list.append(plan)
        return_plan = torch.stack(plan_list)
        final_plan = return_plan*num_seq
        if relation_matrix == None:
            return return_plan
        else:
            if args.loss_only_label1:
                loss_raw = self.criterion(torch.multiply(final_plan, relation_matrix), relation_matrix)
                loss = torch.div(torch.sum(torch.multiply(loss_raw, padding_mask_a)), torch.sum(padding_mask_a))
            else:
                loss_raw = self.criterion(final_plan, relation_matrix)
                loss = torch.div(torch.sum(torch.multiply(loss_raw, padding_mask_a)), torch.sum(padding_mask_a))
            return loss
...
def train_pair(args, model, OT_model, train_dataloader, valid_dataloader, test_dataloader):
    ...
    if args.cost_cal_way == 'attention' or args.cost_cal_way == 'class_embedding':
...
def evaluate(model, OT_model, valid_dataloader, epoch, type='valid'):
    ...
    for batch_data in pbar:
        ...
        plan_list = OT_model(output_batch_A, output_batch_B, x_batch_A, x_batch_B, None,
...
def model_class(batch_A, batch_B, plan_matrix):
    ...
    vec_correct_A = torch.argmax(batch_A, dim=-1) + (relation_A)*torch.ge(torch.argmax(batch_A, dim=-1), 1)
    vec_correct_B = torch.argmax(batch_B, dim=-1) + (relation_B)*torch.ge(torch.argmax(batch_B, dim=-1), 1)
...
def OT_param_search(model, valid_dataloader, test_dataloader):
    ...
    f_mass_list = list(np.arange(3.0, 3.1, 0.1))
    weight_list = [0, 10, 100, 200]
    reg_list = list(np.arange(0.5, 1, 0.1))
        ...
        for f_mass in f_mass_list:
            for weight in weight_list:
                for reg in reg_list:
                    ...
                    #load_checkpoint(OT_model, None, -1,  "/new_disk2/zhongxiang_sun/code/explanation_project/explanation_model/models/weights/extract/extract_ot-criterion-MSE-onehot-1-ot_mode-max-four_class-1-coefficient_lrable-0-convert_to_onehot-1-loss_only_label1-0-weight-10-cost_cal_way-class_embedding.pkl")
    ...
    pd_valid.to_csv("../dataset/our_data/valid_ot_class_embedding_BCEFocal_wo.csv", index=False)
    pd_test.to_csv("../dataset/our_data/test_ot_class_embedding_BCEFocal_wo.csv", index=False)
...
if __name__ == '__main__':
    ...
    for i, d in enumerate(data):
        ...
        for j in case_a[1]:
            if j[0] in important_A:
                data_y_seven_class[2*i, j[0]] = j[1]+1
            else:
                data_y_seven_class[2*i, j[0]] = j[1]

        for j in case_b[1]:
            if j[0] in important_B:
                data_y_seven_class[2*i+1, j[0]] = j[1]+1
            else:
    ...
    if args.model_usage == 'train':
        ...
        load_checkpoint(model, None, 22,
                    "/new_disk2/zhongxiang_sun/code/explanation_project/explanation_model/models/weights/extract/extract-criterion-BCEFocal-onehot-1-ot_mode-max-four_class-1-coefficient_lrable-0-convert_to_onehot-1-loss_only_label1-0-weight-10-cost_cal_way-class_embedding.pkl")
        OT_model = OT(reg=args.reg).to(device)
        load_checkpoint(OT_model, None, -1,
                        "/new_disk2/zhongxiang_sun/code/explanation_project/explanation_model/models/weights/extract/extract_ot-criterion-BCEFocal-onehot-1-ot_mode-max-four_class-1-coefficient_lrable-0-convert_to_onehot-1-loss_only_label1-0-weight-10-cost_cal_way-class_embedding.pkl")
